pub fn test_simple_gettime() {
    let now = chrono::Local::now();
    println!("{}", now);
}

pub fn test_complex_gettime() {
    struct Clock;
    impl Clock {
        fn get() -> chrono::DateTime<chrono::Local> {
            chrono::Local::now()
        }
    }

    fn main() {
        let app = clap::App::new("clock")
            .version("0.1")
...
pub fn test_set_time() {
    ...
    fn main() {
        let app = App::new("clock")
            ...
            .about("Gets and (aspirationally) sets the time.")
            .after_help(
                "Note: UNIX timestamps are parsed as whole seconds since 1st January 1970 0:00:00 UTC. For more accuracy, use another format.",
            )
            .arg(
