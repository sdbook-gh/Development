...
static void err_handler(Dwarf_Error err, Dwarf_Ptr errarg) {
  printf("libdwarf error reading %s: %lu %s\n", objfile_name, (unsigned long)dwarf_errno(err), dwarf_errmsg(err));
  if (errarg) { printf("Error: errarg is nonnull but it should be null\n"); }
  printf("Giving up");
  exit(1);
}

static int dwarf5_ranges(Dwarf_Die cu_die, Dwarf_Addr *lowest, Dwarf_Addr *highest);
static int dwarf4_ranges(Dwarf_Debug dbg, Dwarf_Die cu_die, Dwarf_Addr cu_lowpc, Dwarf_Addr *lowest, Dwarf_Addr *highest);
...
static Dwarf_Bool pc_in_die(Dwarf_Debug dbg, Dwarf_Die die, int version, Dwarf_Addr pc) {
  ...
  if (ret == DW_DLV_OK) {
    if (pc == cu_lowpc) { return TRUE; }
    ret = dwarf_highpc_b(die, &cu_highpc, NULL, &highpc_cls, NULL);
    if (ret == DW_DLV_OK) {
      if (highpc_cls == DW_FORM_CLASS_CONSTANT) { cu_highpc += cu_lowpc; }
      if (pc >= cu_lowpc && pc < cu_highpc) { return TRUE; }
    }
  }
  ...
  if (pc >= lowest && pc < highest) { return TRUE; }
  return FALSE;
}
...
static void print_line(Dwarf_Debug dbg, flagsT *flags, Dwarf_Line line, Dwarf_Addr pc) {
  ...
  if (flags->addresses) { printf("%#018" DW_PR_DUx "\n", pc); }
  if (line) {
...
static Dwarf_Bool lookup_pc_cu(Dwarf_Debug dbg, flagsT *flags, Dwarf_Addr pc, Dwarf_Die cu_die) {
  int ret;
  ...
  if (ret == DW_DLV_NO_ENTRY) { return FALSE; }
  if (table_count == 1) {
    ...
    for (int i = 0; i < linecount; i++) {
      ...
      if (pc == lineaddr) {
        ...
        for (int j = i + 1; j < linecount; j++) {
          ...
          if (pc == lineaddr) { last_pc_line = j_line; }
        }
        is_found = TRUE;
...
static Dwarf_Bool lookup_pc(Dwarf_Debug dbg, flagsT *flags, Dwarf_Addr pc) {
  ...
  for (int cu_i = 0;; cu_i++) {
    ret = dwarf_next_cu_header_d(dbg, is_info, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, &next_cu_header, &header_cu_type, NULL);
    ...
    if (ret == DW_DLV_OK) {
      ...
      if (pc_in_die(dbg, cu_die, dwversion, pc)) {
        Dwarf_Bool lookup_ret = lookup_pc_cu(dbg, flags, pc, cu_die);
        dwarf_dealloc_die(cu_die);
        while (dwarf_next_cu_header_d(dbg, is_info, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, &next_cu_header, &header_cu_type, NULL) != DW_DLV_NO_ENTRY) {}
        return lookup_ret;
      } else {
        dwarf_dealloc_die(cu_die);
        cu_die = 0;
...
static int get_rnglist_offset(Dwarf_Attribute attr, Dwarf_Unsigned *offset) {
...
static int dwarf5_ranges(Dwarf_Die cu_die, Dwarf_Addr *lowest, Dwarf_Addr *highest) {
  ...
  if (get_rnglist_offset(attr, &offset) == DW_DLV_OK) {
    ...
    /* offset either a global offset (FORM == DW_FORM_sec_offset)
            ...
            of the details */
    res = dwarf_rnglists_get_rle_head(attr, attrform, offset, &head, &rnglists_count, &rlesetoffset, NULL);
    if (res != DW_DLV_OK) {
    ...
    for (; i < rnglists_count; ++i) {
      ...
      res = dwarf_get_rnglists_entry_fields_a(head, i, &entrylen, &rle_val, &raw1, &raw2, &unavail, &cooked1, &cooked2, NULL);
      if (res != DW_DLV_OK) {
        /* ASSERT: is DW_DLV_NO_ENTRY */
        continue;
      }
      ...
      switch (rle_val) {
        ...
        case DW_RLE_start_length:
          if (cooked1 < *lowest) { *lowest = cooked1; }
          if (cooked2 > *highest) { *highest = cooked2; }
        default:
          /* Something is wrong. */
          break;
      }
...
static int dwarf4_ranges(Dwarf_Debug dbg, Dwarf_Die cu_die, Dwarf_Addr cu_lowpc, Dwarf_Addr *lowest, Dwarf_Addr *highest) {
  ...
  if (dwarf_global_formref(attr, &offset, NULL) == DW_DLV_OK) {
    ...
    if (cu_lowpc != HIGHADDR) { baseaddr = cu_lowpc; }
    res = dwarf_get_ranges_b(dbg, offset, cu_die, NULL, &ranges, &count, NULL, NULL);
    for (int i = 0; i < count; i++) {
      ...
      if (cur->dwr_type == DW_RANGES_ENTRY) {
        ...
        if (rng_lowpc < *lowest) { *lowest = rng_lowpc; }
        if (rng_highpc > *highest) { *highest = rng_highpc; }
...
static Dwarf_Bool get_pc_range(Dwarf_Debug dbg, Dwarf_Addr *lowest, Dwarf_Addr *highest, int *cu_cnt) {
  Dwarf_Bool is_info = TRUE;
  Dwarf_Unsigned next_cu_header;
  Dwarf_Half header_cu_type;
  Dwarf_Half dwversion = 0;
  ...
  *highest = 0;
  ...
  for (cu_i = 0;; cu_i++) {
    ...
    if (ret == DW_DLV_OK) {
      ...
      if (ret == DW_DLV_OK) {
        if (cu_lowpc < *lowest) { *lowest = cu_lowpc; }
        ret = dwarf_highpc_b(cu_die, &cu_highpc, NULL, &highpc_cls, NULL);
        if (ret == DW_DLV_OK) {
          if (highpc_cls == DW_FORM_CLASS_CONSTANT) { cu_highpc += cu_lowpc; }
          if (cu_highpc > *highest) { *highest = cu_highpc; }
        }
      }
      if (dwversion >= DWARF5_VERSION) {
        ret = dwarf5_ranges(cu_die, lowest, highest);
      } else {
        ret = dwarf4_ranges(dbg, cu_die, cu_lowpc, lowest, highest);
      }
      if (ret == DW_DLV_ERROR) { return 0; }
      dwarf_dealloc_die(cu_die);
      cu_die = 0;
    }
  }
...
static void populate_lookup_table_die(lookup_tableT *lookup_table, int cu_i, Dwarf_Die cu_die) {
  int ret;
  Dwarf_Unsigned version;
  Dwarf_Small table_count;
  ...
  if (table_count == 1) {
    ...
    ret = dwarf_srclines_from_linecontext(lookup_table->ctxts[cu_i], &linebuf, &linecount, NULL);
    ...
    for (int i = 0; i < linecount; i++) {
      ...
      if (ret == DW_DLV_NO_ENTRY) { continue; }
      if (prev_line) {
        for (Dwarf_Addr addr = prev_lineaddr; addr < lineaddr; addr++) { lookup_table->table[addr - lookup_table->low] = linebuf[i - 1]; }
      }
      Dwarf_Bool is_lne = 0;
...
static void populate_lookup_table(Dwarf_Debug dbg, lookup_tableT *lookup_table) {
  ...
  for (int cu_i = 0;; cu_i++) {
    ...
    ret = dwarf_siblingof_b(dbg, 0, is_info, &cu_die, NULL);
    if (ret == DW_DLV_OK) {
      populate_lookup_table_die(lookup_table, cu_i, cu_die);
...
static int create_lookup_table(Dwarf_Debug dbg, lookup_tableT *lookup_table) {
  ...
  if (!get_pc_range(dbg, &low, &high, &cu_cnt)) { goto exit; }
  lookup_table->table = (Dwarf_Line_s **)malloc((high - low) * sizeof(Dwarf_Line));
  if (!lookup_table->table) { goto exit; }
  lookup_table->ctxts = (Dwarf_Line_Context_s **)malloc(cu_cnt * sizeof(Dwarf_Line_Context));
  if (!lookup_table->ctxts) { goto free_table_exit; }
...
static void delete_lookup_table(lookup_tableT *lookup_table) {
  free(lookup_table->table);
  lookup_table->table = NULL;
  for (int i = 0; i < lookup_table->cnt; i++) { dwarf_srclines_dealloc_b(lookup_table->ctxts[i]); }
  free(lookup_table->ctxts);
  lookup_table->ctxts = NULL;
}

static char *get_pc_buf(int argc, char **argv, char *buf, Dwarf_Bool do_read_stdin) {
  if (do_read_stdin) {
...
char *usagestrings[] = {"addr2line [ -h] [-a] [-e <objectpath>] [-b] [-n] [address] ...", "where", "-a --addresses  Turns on printing of address before", "    the source line text", "-e --exe  <path> The the path to the object file to read.", "    Path defaults to \"a.out\"", "-b --force-batch The CU address ranges will be looked", "    up once at the start and the generated table used.", "-n --force-no-batch The addresses are looked up", "    independently for each address present.", "    In certain cases the no-batch will be overridden", "    and batching used.", "-h --help  Prints a help message and stops.", 0};
...
static void populate_options(int argc, char *argv[], char **objfile, flagsT *flags) {
  ...
  while (1) {
    int option_index = 0;
    static struct option longopts[] = {{"addresses", no_argument, 0, 'a'}, {"exe", required_argument, 0, 'e'}, {"help", no_argument, 0, 'h'}, {"force-batch", no_argument, 0, 'b'}, {"force-no-batch", no_argument, 0, 'n'}, {0, 0, 0, 0}};
    c = getopt_long(argc, argv, "ae:hbn", longopts, &option_index);
    if (c == -1) { break; }
    switch (c) {
      case 'a':
...
int main(int argc, char *argv[]) {
  ...
  ret = dwarf_init_path(objfile, NULL, 0, DW_GROUPNUMBER_ANY, err_handler, errarg, &dbg, NULL);
  if (ret == DW_DLV_NO_ENTRY) { fail_exit("Unable to open file"); }

  do_read_stdin = (optind >= argc);
  if (!flags.force_nobatchmode && (flags.force_batchmode || do_read_stdin || (argc + BATCHMODE_HEURISTIC) > optind)) { flags.batchmode = TRUE; }
  if (flags.batchmode) {
    ret = create_lookup_table(dbg, &lookup_table);
    if (ret != DW_DLV_OK) { fail_exit("Unable to create lookup table"); }
  }
  ...
  while ((pc_buf = get_pc_buf(argc, argv, buf, do_read_stdin))) {
    ...
    if (!is_found) { print_line(dbg, &flags, NULL, pc); }
  }
  if (flags.batchmode && lookup_table.table) { delete_lookup_table(&lookup_table); }
  dwarf_finish(dbg);
  return 0;
}
