...
void reverse_full_string(std::string& in_str) {
  ...
  if (start_pos < in_str.length()) { reverse_string(in_str, start_pos, in_str.length()); }
}
...
void print_vector(const std::vector<T>& v) {
  std::for_each(v.begin(), v.end(), [](const T& val) { std::cout << val << " "; });
  std::cout << std::endl;
}
#include <vector>
template <typename T>
std::vector<T> merge(const std::vector<T>& v1, const std::vector<T>& v2) {
  ...
  if (pos1 < v1.size()) { v.insert(v.end(), v1.begin() + pos1, v1.end()); }
  if (pos2 < v2.size()) { v.insert(v.end(), v2.begin() + pos2, v2.end()); }
  return v;
}
template <typename T>
std::vector<T> merge_new(const std::vector<T>& in_v, T in_val) {
  ...
  for (auto i = 0; i < pos_v.size(); ++i) { v.emplace_back(in_v[pos_v[i]]); }
  return v;
}
int test_merge() {
  {
...
public:
  ...
  bool reverse_range(size_t begin, size_t end) {
    if (begin < 0) { printf("bad begin < 0\n"); }
    if (begin > _size - 1) { printf("bad begin > _size - 1\n"); }
    if (end < 0) { printf("bad end < 0\n"); }
    if (end > _size - 1) { printf("bad end > _size - 1\n"); }
    if (end < begin) { printf("bad end < begin\n"); }
    auto e_b = _head;
    for (auto i = 0; i < begin; ++i) { e_b = e_b->_next; }
    auto e_e = e_b;
    for (auto i = begin; i < end; ++i) { e_e = e_e->_next; }
    auto head = e_b->_prev;
    auto tail = e_e->_next;
    auto node = std::make_shared<Node<T>>();
    ...
    if (head != nullptr) { head->_next = e_e; }
    e_e->_prev = head;
    if (tail != nullptr) { tail->_prev = e_b; }
    e_b->_next = tail;
    if (head == nullptr) { _head = e_e; }
    if (tail == nullptr) { _tail = e_b; }
    return true;
  }
  std::string debug() {
    std::stringstream ss;
    ss << "[" << _size << "] ";
    for (auto e = _head; e != nullptr; e = e->_next) { ss << e->_value << " "; }
    return ss.str();
  }
};
int test_list() {
...
struct TestClass {
  void test_func(int a) { printf("int %d\n", a); }
  void test_func(long a) { printf("long %ld\n", a); }
  int val;
};
...
int main() {
  ...
  auto ptr1 = static_cast<void (TestClass::*)(int)>(&TestClass::test_func);
  auto ptr2 = static_cast<void (TestClass::*)(long)>(&TestClass::test_func);
  auto ptr3 = &TestClass::val;
  TestClass tc;
  (tc.*ptr1)(1);
  (tc.*ptr2)(1);
  tc.*ptr3 = 10;
  ...
  // 将ptr3转换为uint64_t类型
  uint64_t val;
  memcpy(&val, &ptr1, sizeof(uint64_t));
  printf("0x%lx\n", val);
  memcpy(&val, &ptr2, sizeof(uint64_t));
  printf("0x%lx\n", val);
}
