...
class Cgroup(object):
    ...
    def set_cpu_share(self, share=None):
        if 'cpu' in self.cgroups:
            value = self._format_cpu_share_value(share)
    ...
    def _format_cpu_limit_value(self, limit=None):
        if limit is None:
            value = CPU_DEFAULT_US
        else:
            try:
                value = CPU_DEFAULT_US * int(limit)
            except ValueError:
                raise CgroupsException('Limit must be convertible to a int')
            else:
                if limit <= int(0) or limit > int(100):
                    raise CgroupsException('Limit must be between 0 and 100')
                else:
                    limit = limit / 100
                    value = int(round(CPU_DEFAULT_US * limit))
        return value

    def set_cpu_limit(self, limit=None):
        if 'cpu' in self.cgroups:
            value = self._format_cpu_limit_value(limit)
            cpu_limit_file = self._get_cgroup_file('cpu', 'cpu.cfs_quota_us')
            with open(cpu_limit_file, 'w+') as f:
                f.write('%s\n' % value)
        else:
            raise CgroupsException(
                'CPU hierarchy not available in this cgroup')
