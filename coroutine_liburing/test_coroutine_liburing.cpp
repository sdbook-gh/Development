...
public:
  ...
  ReadOnlyFile(ReadOnlyFile &&other)
      : path_{std::exchange(other.path_, {})}, fd_{std::exchange(other.fd_, -1)}, size_{other.size()} {};
  ~ReadOnlyFile() {
    if (fd_) {
      close(fd_);
    }
...
std::vector<std::vector<char>> initializeBuffers(const std::vector<ReadOnlyFile> &files) {
...
void readObjFromBuffer(const std::vector<char> &buff, std::string &reader) {
  auto s = std::string(buff.data(), buff.size());
  reader = "content:" + s;
}

void pushEntriesToSubmissionQueue(const std::vector<ReadOnlyFile> &files, std::vector<std::vector<char>> &buffs,
                                  IOUring &uring) {
...
std::vector<Result> readEntriesFromCompletionQueue(const std::vector<ReadOnlyFile> &files,
                                                   const std::vector<std::vector<char>> &buffs, IOUring &uring) {
...
std::vector<Result> thread_pool(const std::vector<ReadOnlyFile> &files) {
  ...
  BS::thread_pool pool;
  pool.submit_loop<size_t>(0, files.size(), [&files, &result](int i) { result[i] = readSynchronous(files[i]); }).wait();
  return result;
}
...
bool isGood(const Result &result) {
  if (result.status_code < 0) {
    std::cout << "Error reading file: " << result.file << ". Error: " << result.status_code << "\n";
    return false;
  }
  if (result.result.empty()) {
    std::cerr << "Error parsing file: " << result.file << ". Reason: "
              << "empty"
              << "\n";
    return false;
  }
...
public:
  struct promise_type {
    ...
    void return_value(Result result) noexcept { this->result = std::move(result); }
  ...
  auto schedule() {
    struct Awaiter : std::suspend_always {
      ...
      void await_suspend(std::coroutine_handle<> handle) {
        tpool.detach_task([handle]() { handle.resume(); });
      }
    };
...
bool allDone(const std::vector<Task> &tasks) {
  return std::all_of(tasks.cbegin(), tasks.cend(), [](const auto &t) { return t.done(); });
}
...
int main(int argc, char *argv[]) {
  if (argc < 3) {
    std::cout << "please add scan path and execution mode to parameter"
              << "\n";
    exit(1);
  }
  ...
  auto results = [&files](std::string cmd) {
    if (cmd == "--coroutines") {
      std::cout << "Running couroutines implementation"
                << "\n";
      return coroutines(files);
    } else if (cmd == "--iouring") {
      std::cout << "Running iouring implementation"
                << "\n";
      return iouring_only(files);
    } else if (cmd == "--thread-pool") {
      std::cout << "Running multi-threading implementation"
                << "\n";
      return thread_pool(files);
    } else if (cmd == "--coro-pool") {
      std::cout << "Running coroutines thread pool implementation"
                << "\n";
      return coroutines_pool(files);
    } else if (cmd == "--trivial") {
      std::cout << "Running trivial implementation"
                << "\n";
      return trivial(files);
    } else {
      std::cout << "bad execution mode"
                << "\n";
      exit(1);
    }
  }(argv[2]);
  ...
  for (const auto &result : results) {
    if (isGood(result) && !result.result.empty()) {
    }
    ++processed;
  }
