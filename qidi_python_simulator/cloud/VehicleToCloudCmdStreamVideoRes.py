...
class VehicleToCloudCmdStreamVideoRes:
    __logger = logging.getLogger('VehicleToCloudCmdStreamVideoRes')
    ...
    def parse_buffer(self, buffer, start_pos: int) -> int:
        ...
        VehicleToCloudCmdStreamVideoRes.__logger.debug(f'parse get cmdType_var:{cmdType_var}')
        pos += struct.calcsize('B')
        uuid_var = struct.unpack_from('36s', buffer, pos)
        VehicleToCloudCmdStreamVideoRes.__logger.debug(f'parse get uuid_var:{uuid_var}')
        pos += struct.calcsize('36s')
        doFlag_var = struct.unpack_from('B', buffer, pos)
        VehicleToCloudCmdStreamVideoRes.__logger.debug(f'parse get doFlag_var:{doFlag_var}')
        pos += struct.calcsize('B')
        return pos
    ...
    def fill_buffer(self, buffer: bytes, start_pos: int) -> int:
        ...
        VehicleToCloudCmdStreamVideoRes.__logger.debug(f'fill cmdType_var:{cmdType_var}')
        pos += struct.calcsize('B')
        uuid_var = self.m_uuid
        struct.pack_into(Constant.CLOUD_PROTOCOL_ENDIAN_SIGN + '36s', buffer, pos, uuid_var)
        VehicleToCloudCmdStreamVideoRes.__logger.debug(f'fill uuid_var:{uuid_var}')
        pos += struct.calcsize('36s')
        doFlag_var = self.m_doFlag
        struct.pack_into(Constant.CLOUD_PROTOCOL_ENDIAN_SIGN + 'B', buffer, pos, doFlag_var)
        VehicleToCloudCmdStreamVideoRes.__logger.debug(f'fill doFlag_var:{doFlag_var}')
        pos += struct.calcsize('B')
        return pos
