...
class CloudToVehicleCmdLaneSpdLmtRes:
    __logger = logging.getLogger('CloudToVehicleCmdLaneSpdLmtRes')
    ...
    def parse_buffer(self, buffer, start_pos: int) -> int:
        ...
        CloudToVehicleCmdLaneSpdLmtRes.__logger.debug(f'parse get cmdType_var:{cmdType_var}')
        pos += struct.calcsize('B')
        seq_var = struct.unpack_from('Q', buffer, pos)
        CloudToVehicleCmdLaneSpdLmtRes.__logger.debug(f'parse get seq_var:{seq_var}')
        pos += struct.calcsize('Q')
        doFlag_var = struct.unpack_from('B', buffer, pos)
        CloudToVehicleCmdLaneSpdLmtRes.__logger.debug(f'parse get doFlag_var:{doFlag_var}')
        pos += struct.calcsize('B')
        return pos
    ...
    def fill_buffer(self, buffer: bytes, start_pos: int) -> int:
        ...
        CloudToVehicleCmdLaneSpdLmtRes.__logger.debug(f'fill cmdType_var:{cmdType_var}')
        pos += struct.calcsize('B')
        seq_var = self.m_seq
        struct.pack_into(Constant.CLOUD_PROTOCOL_ENDIAN_SIGN + 'Q', buffer, pos, seq_var)
        CloudToVehicleCmdLaneSpdLmtRes.__logger.debug(f'fill seq_var:{seq_var}')
        pos += struct.calcsize('Q')
        doFlag_var = self.m_doFlag
        struct.pack_into(Constant.CLOUD_PROTOCOL_ENDIAN_SIGN + 'B', buffer, pos, doFlag_var)
        CloudToVehicleCmdLaneSpdLmtRes.__logger.debug(f'fill doFlag_var:{doFlag_var}')
        pos += struct.calcsize('B')
        return pos
