...
class CloudToVehicleCmdNtlar:
    __logger = logging.getLogger('CloudToVehicleCmdNtlar')
    ...
    def parse_buffer(self, buffer, start_pos: int) -> int:
        ...
        CloudToVehicleCmdNtlar.__logger.debug(f'parse get cmdType_var:{cmdType_var}')
        pos += struct.calcsize('B')
        uuid_var = struct.unpack_from('36s', buffer, pos)
        CloudToVehicleCmdNtlar.__logger.debug(f'parse get uuid_var:{uuid_var}')
        pos += struct.calcsize('36s')
        seq_var = struct.unpack_from('Q', buffer, pos)
        CloudToVehicleCmdNtlar.__logger.debug(f'parse get seq_var:{seq_var}')
        pos += struct.calcsize('Q')
        cmdFlag_var = struct.unpack_from('B', buffer, pos)
        CloudToVehicleCmdNtlar.__logger.debug(f'parse get cmdFlag_var:{cmdFlag_var}')
        pos += struct.calcsize('B')
        level_var = struct.unpack_from('B', buffer, pos)
        CloudToVehicleCmdNtlar.__logger.debug(f'parse get level_var:{level_var}')
        pos += struct.calcsize('B')
        return pos
    ...
    def fill_buffer(self, buffer: bytes, start_pos: int) -> int:
        ...
        CloudToVehicleCmdNtlar.__logger.debug(f'fill cmdType_var:{cmdType_var}')
        pos += struct.calcsize('B')
        uuid_var = self.m_uuid
        struct.pack_into(Constant.CLOUD_PROTOCOL_ENDIAN_SIGN + '36s', buffer, pos, uuid_var)
        CloudToVehicleCmdNtlar.__logger.debug(f'fill uuid_var:{uuid_var}')
        pos += struct.calcsize('36s')
        seq_var = self.m_seq
        struct.pack_into(Constant.CLOUD_PROTOCOL_ENDIAN_SIGN + 'Q', buffer, pos, seq_var)
        CloudToVehicleCmdNtlar.__logger.debug(f'fill seq_var:{seq_var}')
        pos += struct.calcsize('Q')
        cmdFlag_var = self.m_cmdFlag
        struct.pack_into(Constant.CLOUD_PROTOCOL_ENDIAN_SIGN + 'B', buffer, pos, cmdFlag_var)
        CloudToVehicleCmdNtlar.__logger.debug(f'fill cmdFlag_var:{cmdFlag_var}')
        pos += struct.calcsize('B')
        level_var = self.m_level
        struct.pack_into(Constant.CLOUD_PROTOCOL_ENDIAN_SIGN + 'B', buffer, pos, level_var)
        CloudToVehicleCmdNtlar.__logger.debug(f'fill level_var:{level_var}')
        pos += struct.calcsize('B')
        return pos
