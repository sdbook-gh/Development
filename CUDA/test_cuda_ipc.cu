...
void runTestMultiKernel(ipcCUDA_t *s_mem, int index) {
    ...
    if (index == 0) {
        ...
        checkCudaErrors(cudaMemcpy(h_results, d_ptr + DATA_BUF_SIZE, DATA_BUF_SIZE * (g_processCount - 1) * sizeof(int),
        ...
        checkCudaErrors(cudaIpcOpenMemHandle((void **)&d_ptr, s_mem[0].memHandle, cudaIpcMemLazyEnablePeerAccess));
        printf(
            "> Process %3d: Run kernel on GPU%d, taking source data from and writing results to process %d, GPU%d...\n",
...
int main(int argc, char **argv) {
    ...
    if (!IsAppBuiltAs64()) {
        printf(
            "%s is only supported on 64-bit Linux OS and the application must be built as a 64-bit target. Test is "
            "being waived.\n",
            argv[0]);
        exit(EXIT_WAIVED);
    }
    ...
    ipcDevices_t *s_devices =
        (ipcDevices_t *)mmap(NULL, sizeof(*s_devices), PROT_READ | PROT_WRITE, MAP_SHARED | MAP_ANONYMOUS, 0, 0);
    ...
    g_barrier =
        (ipcBarrier_t *)mmap(NULL, sizeof(*g_barrier), PROT_READ | PROT_WRITE, MAP_SHARED | MAP_ANONYMOUS, 0, 0);
    ...
    ipcCUDA_t *s_mem = (ipcCUDA_t *)mmap(NULL, g_processCount * sizeof(*s_mem), PROT_READ | PROT_WRITE,
