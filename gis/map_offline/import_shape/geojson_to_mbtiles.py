...
使用 tippecanoe 按图层分别生成瓦片，再用 tile-join 合并为各省 mbtiles。

并发模型:
  - ProcessPoolExecutor 运行 tippecanoe (CPU 密集)
  - ThreadPoolExecutor 运行 tile-join (I/O 等待)
  - 两个池互不阻塞，主线程只做调度
  - 每 60 秒输出一次各 worker 状态

用法:
    python geojson_to_mbtiles.py -i ./geojson_output -o ./mbtiles_output -w 8
...
def scan_subdirs(input_dir: Path) -> list[Path]:
    """扫描 input_dir 下所有包含 .geojson 的子目录."""
    subdirs = sorted(
...
def run_tippecanoe(args_tuple: tuple) -> dict:
    ...
    注意: _inflight 状态由主线程在提交/完成时管理，不在此处调用，
    因为子进程无法共享主进程的内存。
    """
    (subdir, short_name, layer_name, layer_cfg,
     temp_dir, global_args, dry_run) = args_tuple
    ...
    cmd = ["tippecanoe"] + global_args
    cmd += ["-Z", str(layer_cfg["zoom_min"]), "-z", str(layer_cfg["zoom_max"])]
    cmd += ["-l", layer_name]
    cmd += ["-o", str(mbtiles_path)]
    cmd += layer_cfg.get("tippecanoe_args", [])
    cmd += [str(geojson_path)]

    if dry_run:
        print(f"[DRY-RUN] {province}/{layer_name}: {' '.join(cmd)}")
        return {"province": province, "short_name": short_name,
    ...
    if r.returncode != 0:
        return {"province": province, "short_name": short_name,
                ...
                "error": r.stderr.strip()[-300:], "elapsed": elapsed}
...
def tile_join_worker(short_name: str, temp_dir: Path, output_root: Path,
                     join_args: list[str], ok_count: int, total: int) -> dict:
    """线程池 worker: 执行 tile-join. 线程共享主进程内存，可直接写 _inflight."""
    key = f"tj:{short_name}"
    ...
    if mbtiles_files:
        join_cmd = ["tile-join"] + join_args
        join_cmd += ["-o", str(output_mbtiles)]
        join_cmd += [str(f) for f in mbtiles_files]

        r = subprocess.run(join_cmd, capture_output=True, text=True)
        elapsed = time.time() - start
...
def main():
    parser = argparse.ArgumentParser(description="GeoJSON → mbtiles 按省份并发转换")
    parser.add_argument("-c", "--config", default="geojson_tile.json",
                        help="JSON 配置文件路径, 默认 geojson_tile.json")
    ...
    province_temp_dir = {}  # short_name → temp_dir Path
    ...
    join_futures = {}  # future → short_name
    ...
    with ProcessPoolExecutor(max_workers=workers) as executor:
        ...
        # 主线程记录所有 tippecanoe in-flight 任务
        ...
        for future in as_completed(futures):
            ...
            short_name = task[1]
            layer_name = task[2]
            province_temp_dir[short_name] = task[4]  # temp_dir

            # 从 in-flight 移除 (tippecanoe 已完成)
    ...
    # 跳过 tippecanoe，直接提交 tile-join
    # for sd in subdirs:
    #   short_name = name_map[sd.name]
    #   temp_dir = temp_root / short_name
    #   if temp_dir.exists() and list(temp_dir.glob("*.mbtiles")):
    #       fut = join_executor.submit(
    #           tile_join_worker, short_name, temp_dir,
    #           output_root, join_args, total_per_province, total_per_province)
    #       join_futures[fut] = short_name
    #       province_temp_dir[short_name] = temp_dir
    #       print(f"  [{short_name}] tile-join 已提交")
